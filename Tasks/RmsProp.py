...
def update_parameters_with_rms(parameters, gradients, epoch, v, s, t, learning_rate=0.007, beta1=0.9, beta2=0.999, epsilon=1e-8, decay_rate = 0.01):
    ...
    for l in range(L):
        ...
        s["dW" + str(l + 1)] = beta1 * s["dW" + str(l + 1)] + (1 - beta1) * np.power(gradients['dW' + str(l + 1)], 2)
        s["db" + str(l + 1)] = beta1 * s["db" + str(l + 1)] + (1 - beta1) * np.power(gradients['db' + str(l + 1)], 2)
        
        # Update parameters. Inputs: "parameters, learning_rate, v_corrected, s_corrected, epsilon". Output: "parameters".

        parameters["W" + str(l + 1)] = parameters["W" + str(l + 1)] - learning_rate * gradients['dW' + str(l + 1)]  / np.sqrt(s["dW" + str(l + 1)] + epsilon)
        parameters["b" + str(l + 1)] = parameters["b" + str(l + 1)] - learning_rate * gradients['db' + str(l + 1)]  / np.sqrt(s["db" + str(l + 1)] + epsilon)
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.007, decay_rate=0.01, batch_size = 10):
    ...
    for ii in range(num_iterations):
        for index in range(n_batches):
            ...
            t = t + 1 # Adam counter
            parameters, alpha, v, s = update_parameters_with_rms(parameters, gradients, ii, v, s, t, learning_rate, beta1, beta2,  epsilon, decay_rate)
...
def main():
    ...
    for x in learning_rate:
        ...
        costs, val_loss, parameters = multi_layer_network(train_data, train_label, net_dims, num_iterations=num_iterations, learning_rate=x,  decay_rate=decay_rate , batch_size = batch_size)
        ...
        plt.title("Loss vs Number of Iterations with "+str(x)+" Learning rate")
